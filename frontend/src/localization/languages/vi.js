import { LocalizationKey } from "../keys";

export const vi = {
  [LocalizationKey.WELCOME]: "Xin chào",
  [LocalizationKey.LOGIN]: "Đăng nhập",
  [LocalizationKey.CURRENCY]: "Tiền tệ",
  [LocalizationKey.LANGUAGE]: "Ngôn ngữ",
  [LocalizationKey.NOTIFICATION]: "Thông báo",
  [LocalizationKey.SECURITY]: "Bảo mật",
  [LocalizationKey.PRIVACY]: "Quyền riêng tư",
  [LocalizationKey.LOGOUT]: "Đăng xuất",

  // Header title
  [LocalizationKey.SETTINGS]: "Cài đặt",
  [LocalizationKey.ACCOUNTS]: "Tài khoản",
  [LocalizationKey.STATISTICS]: "Thống kê",

  // Account
  [LocalizationKey.FULLNAME]: "Họ và tên",
  [LocalizationKey.PASSWORD]: "Mật khẩu",
  [LocalizationKey.BANKACCOUNT]: "Tài khoản ngân hàng",

  // Statistic
  [LocalizationKey.INCOME_REPORT]: "Báo cáo thu nhập",
  [LocalizationKey.EXPENSE_REPORT]: "Báo cáo chi phí",
  [LocalizationKey.DEBT_REPORT]: "Báo cáo nợ",
  [LocalizationKey.RECEIVABLE_REPORT]: "Báo cáo phải thu",
  [LocalizationKey.INCOME_VS_EXPENSE]: "Thu nhập vs Chi phí",
  [LocalizationKey.RECEIVABLE_VS_DEBT]: "Phải thu vs Nợ",

  // Home
  [LocalizationKey.YOUR_BALANCE]: "Số dư của bạn",
  [LocalizationKey.HI]: "Xin chào",
  [LocalizationKey.CASH]: "Tiền mặt",
  [LocalizationKey.BANK_CARD]: "Bank card",
  [LocalizationKey.NOT_AVAILABLE]: "Không có sẵn",
  [LocalizationKey.MONTHLY_BUDGET]: "Ngân sách hàng tháng",
  [LocalizationKey.SPENT]: "đã tiêu",
  [LocalizationKey.LEFT]: "còn lại",
  [LocalizationKey.RECEIVABLE]: "Cho mượn",
  [LocalizationKey.ADD_NEW_WIDGET]: "Thêm tiện ích mới",
  
  //Pop over
  [LocalizationKey.EXPENSE]: "Chi phí",
  [LocalizationKey.DEBT]: "Mượn nợ",
  [LocalizationKey.INCOME]: "Thu nhập",

  //Transaction

};